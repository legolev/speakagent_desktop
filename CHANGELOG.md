...
## Не выпущено — ускорение и точность ядра транскрибации

**Параллельный ASR-конвейер** — `b03380c`. VAD режет запись в своём потоке, сегменты
декодируют N независимых декодеров (по 1 потоку ORT; N = физические ядра), сборщик
возвращает порядок. Текст байт-в-байт тот же, что при последовательном декоде. Замер на
4 ядрах (12 мин): GigaAM v3 — 37.8 → 22.6 с (×1.67), Parakeet — 56.0 → 31.6 с (×1.77).
Whisper остаётся последовательным (декодер sherpa не потокобезопасен). Модель грузится
параллельно с декодом аудио. Потоки ORT — по физическим ядрам (на Apple Silicon — P-ядра).

**Точнее привязка слов к спикерам** — `b03380c`. sherpa отдаёт BPE-маркер «▁» уже как
ведущий пробел — слова склеивались по ~3 с одним таймкодом (и двойные пробелы в тексте).
Теперь у каждого слова свои таймкоды: доля слов, отнесённых не тому спикеру, 5.6% → 0.7%
на тестовой записи с 4 голосами.

**Ресемплинг с антиалиасингом** — `e853b7d`. Вместо наивной линейной интерполяции —
windowed-sinc (тот же, что внутри sherpa): mp3/wav/flac/ogg и микрофон диктовки больше не
«заворачивают» частоты выше 8 кГц в речевую полосу. WER 3.49% → 3.09% на записи 22 кГц.
Потоковый декод: пик памяти на часовом 48 кГц файле 849 → 237 МБ; ffmpeg читается потоком.

## v0.3.0 — украшатель текста, правка спикеров, фиксы Windows и macOS

**LLM-украшатель расшифровки (опционально, выкл по умолчанию)** — `73bff0d`, `ea35827`.
Прогон расшифровки через выбранную LLM для правки орфографии/пунктуации/очевидных ослышек
БЕЗ изменения смысла и дословности. По-реплично: структура «SpeakerN [ts]:» и таймкоды
сохраняются побайтово, при расхождении числа строк/длины реплика откатывается к оригиналу
(недеструктивно). Переключатель «Оригинал | Обработанный» на записи; включается в Настройках.
Промпты RU/EN, низкая температура, валидация выхода (по best-practice faithful-очистки ASR).

**Правка спикеров постфактум** — `9973b84`. Ростер спикеров с действием «Объединить с…»
(лечит «размножение» спикеров) и переназначение отдельной реплики другому спикеру. Обе правки
переписывают сам текст «SpeakerN …» → долетают до рендера, экспорта, поиска И LLM-итогов;
старые итоги/обработанный текст сбрасываются (строились по прежней атрибуции).

**Windows-фиксы** — `f7cd4fa`. «Сообщить о проблеме на GitHub» открывала Проводник вместо
браузера (перешли с `explorer` на `ShellExecuteW`); выпадающие списки `<select>` были белыми
с невидимым текстом в WebView2 (форсирована тёмная схема для `option`).

**macOS — сброс разрешений диктовки при обновлении** — `ba07592`. Кнопка «Восстановить
разрешения» (вкладка «Диктовка»): `tccutil reset` застрявших TCC-записей + повторный запрос —
чинит в один клик состояние, где переключатель в Настройках не помогает. CI больше не выпускает
ad-hoc-сборку (корень проблемы) + ассерт «не ad-hoc». Надёжный фикс — Apple Developer ID +
нотаризация — в бэклоге.

**CI / релизы** — `d9cb6ef`. Intel-сборка переведена на кросс-компиляцию на arm64-раннере
(нативные Intel-раннеры дефицитны/выводятся из строя) и помечена best-effort — её провал не
блокирует релиз (выходит с Windows + Apple Silicon). Установщики также публикуются под
стабильными именами → прямые кнопки скачивания в README.
