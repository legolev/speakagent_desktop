...
| 🎙️ **Диктовка (push-to-talk)** | Глобальная горячая клавиша (даже одиночная — напр. правый Shift): зажал → говоришь, отпустил → текст мгновенно распознаётся локально, копируется в буфер и вставляется на курсор в любом приложении. История быстрых распознаваний — отдельной вкладкой. |
...
> Приложение пока без платной подписи разработчика, поэтому при первом запуске система
> покажет предупреждение: macOS — правый клик → «Открыть»; Windows — «Подробнее» → «Всё
> равно выполнить».
...
```bash
# 1) поднять "version" в src-tauri/tauri.conf.json и package.json (напр. 0.1.1)
